...
step_analysis <- function(lm) {      
   
  lm_name   <- deparse(substitute(lm))        # name
  lm_form   <- formula(lm)                    # formula 
  ...
  if (good_form) {
    ...
    cat(lm_name, ": The stepwise regression resulted in an intercept-only model (",
