...
skim_flextable <- function(df, col_width = 1.5) {
  df_skim <- skim(df)  |> select(skim_variable, skim_type, n_missing)
  ft_skim <- flextable(df_skim) |>
    ...
    set_table_properties(width = 1.0, layout = "autofit") |>  # global width
    width(j = 1, width = col_width) |>                        # Adjust column 1
    bold(j = 1, bold = TRUE) |>                               # Bold   column 1
    theme_vanilla() # |>                                      # theme
 #   border_outer(part = "all", border = fp_border(color = "black", width = 1)) |> #  border 
 #   border_inner_h(border = fp_border(color = "black", width = 0.5)) |>           #  horizontal 
 #   border_inner_v(border = fp_border(color = "black", width = 0.5))              #  vertical 
  return(ft_skim)
}
