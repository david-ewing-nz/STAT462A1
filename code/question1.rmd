...
### 2.1 Imperial Units Converted To Metric Units

```{r Imperial Units Converted To Metric Units,eval = TRUE}

# Convert units and remove original columns
bdata_m <- bdata %>%
  mutate(
    speed_kmph = speed * 1.60934,  # Convert miles per hour to kilometers per hour
    dist_m = dist * 0.3048          # Convert feet to meters
  ) %>% 
  select(-c(speed, dist))  # Remove original imperial unit columns

sskim<- skim(bdata_m)
print(sskim)
```

### 2.2 Dataset Split Randomly Into Training (80%) And Test (20%) Sets

```{r bDataset-Split,eval = TRUE}
set.seed(1)
...
### 2.3 Scatter Plot Of Training Data (Extra)

```{r Scatter-Plot, eval=T,echo=F}
...
labs(
  ...
  # font changed to sans
  # customize theme
  theme(
    plot.title = element_text(family = "sans", size = 16, face = "bold", hjust = 0.5),
        plot.subtitle = element_text(family = "sans", size = 12, face = "bold", hjust = 0.5),
...
### 3.1 Code To Calculate for Slope $b_1$ and intercept $b_0$
...
```{r bra_calculate_coefficients,include=TRUE, eval=TRUE}
# Calculate b1 and b0 coefficients
mean_speed <- mean(df_train$speed_kmph)
mean_dist <- mean(df_train$dist_m)

b1 <- sum((df_train$speed_kmph - mean_speed) * (df_train$dist_m - mean_dist)) / 
      sum((df_train$speed_kmph - mean_speed)^2)

b0 <- mean_dist - b1 * mean_speed

# Print the coefficients with formatting
cat(paste0("The coefficient b1 is: ", round(b1, 4), "\n"))
cat(paste0("The coefficient b0 is: ", round(b0, 4), "\n"))


```

### 3.2 In this linear regression model: If you increase your speed by 5km/h, how many more metres of braking distance do you expect?:

The slope $b_1$ indicates the change in braking distance for each additional kilometer per hour of speed, and we see that this is a positive relationship. Therefore, in this linear regression model, if you increase your speed by 5km/h, this model predicts that you will need 3.82m (2 d.p.) (0.76\*5) of additional braking distance, with everything else being equal. See code below
...
### 3.3 How much of the variation in the data can be explained by your linear regression model?
...
### 3.4 Is speed a significant predictor for 'dist' at the 95% confidence interval
...
```{r bra_Calculate the standard error of b1,eval=TRUE,include=T}
# Calculate the standard error of b1
n <- nrow(df_train)  # Number of observations
TSS <- sum((df_train$dist_m - mean(df_train$dist_m))^2)  # Total Sum of Squares
ESS_slr <- sum((df_train$dist_m - (b0 + b1 * df_train$speed_kmph))^2)  # Error Sum of Squares

se_b1 <- sqrt(1/(n - 2) * (TSS - ESS_slr)) / sum((df_train$speed_kmph - mean(df_train$speed_kmph))^2)

# Print the standard error with formatting
cat(paste0("The standard error of b1 is: ", round(se_b1, 4), "\n"))
...
### 3.5 Using your linear regression model, predict the braking distance for a car going at 30 km/h, and include an 80% prediction interval
...
**Based on this linear regression model, it can be predicted that the braking distance for a car going at 30 km/h is between 7.60 m and 26.80 m with 80% confidence.**

## 4. Fit a k-NN model to the training set.
...
### 4.1 Predict the braking distance for a car going at 30 km/h using this model.
...
### 4.2 Visualise both models in a graph that also shows the dataset.
...
```{r eval=TRUE, include=TRUE, echo=F}
...
### 4.3 Compare the perfomance of both the k-NN model and the linear regression model (SLR) on the test set by computing both their test set MSE. Which one is performing better?
...
### 4.4 Visualisation of Both Models (Test Dataset)
...
MSEbar <- ggplot(data = dfMSE, aes(x=Model, y=MSE)) + 
  ...
  theme(
    plot.title = element_text(family = "sans", size = 16, face = "bold", hjust = 0.5),
        plot.subtitle = element_text(family = "sans", size = 16, face = "bold", hjust = 0.5),
    axis.title.x = element_text(family = "sans", size = 12),
    axis.title.y = element_text(family = "sans", size = 12),
    axis.text = element_text(family = "sans", size = 10),
    legend.text = element_text(family = "sans", size = 10),
