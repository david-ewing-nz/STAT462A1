...
```{r Prediction Intervals for b1_hat of Models, include=TRUE,eval=T}
results <- data.frame(
...
```{r Display each results table,include=TRUE,eval=TRUE}
...
$\hat{income}=\hat{b}_0+\hat{b}_1 \cdot children$
...
```{r plot trained model, include=TRUE,eval=F}
# Plot both models' fitted lines on income_0 data
ggplot(income_0, aes(x = children, y = income)) +
  geom_point(color = "gray60", alpha = 0.5) +  # All points
  
  # xmodel_0 line
  geom_abline(
    intercept = coef(xmodel_0$model)[1], 
    slope = coef(xmodel_0$model)[2],
    color = "blue", size = 1, linetype = "dashed"
  ) +
  
  # xmodel_1 line
  geom_abline(
    intercept = coef(xmodel_1$model)[1], 
    slope = coef(xmodel_1$model)[2],
    color = "red", size = 1
  ) +
  
  labs(
    title = "Comparison of Linear Models: With and Without Outliers",
  ...
  annotate("text", x = max(income_0$children) * 0.7, y = max(income_0$income) * 0.9,
           label = paste0("R² (with outliers) = ", round(r_squared_0, 3), "\n",
                          "R² (without outliers) = ", round(r_squared_1, 3)),
