...
possum_summary_ft <-  flextable(possum_summary) |>
  ...
  theme_vanilla() |>
  width(j = 1, width = 1.5) |>  # Manually set column widths
  width(j = 2:6, width = 1.2)   # Adjust other columns
```
...
one_hot_columns <- grep("^site", names(possums_1), value = TRUE)
pnumerics <- possums_1 |> select(where(is.numeric), -all_of(one_hot_columns), -sex)
pnumeric_names <- pnumerics |> names()

# Convert possums dataset to long format for faceting
possums_long <- possums_1 |>
  pivot_longer(cols = all_of(pnumeric_names), names_to = "Variable", values_to = "Value") |>
  drop_na(Value)  # Remove missing values to avoid warnings

# Create histogram and density plots
phistograms <- ggplot(possums_long, aes(x = Value)) +
...
phistograms
```
```{r possum-correlation-looped, eval=T}
# Define filter conditions: sex = 1, sex = 0, and no filter
sex_conditions <- list(
  "Sex = 1 (Female)" = possums_1 |> filter(sex == 1),
  "Sex = 0 (Male)" = possums_1 |> filter(sex == 0),
  "All Data (Ignoring Sex)" = possums_1  # No filtering
)

# Loop through each condition
for (condition_name in names(sex_conditions)) {
  
  # Select appropriate dataset based on condition
  subset_data <- sex_conditions[[condition_name]]
  
  # Select numeric features, excluding one-hot site variables
  pnumerics <- subset_data |> 
    select(where(is.numeric), -all_of(one_hot_columns), -sex)
  ...
  print(paste("Highly correlated predictors for:", condition_name))
  ...
  p_correlation_heatmap <- ggplot(to_keep, aes(x = Variable1, y = Variable2, fill = Correlation)) +
    ...
    theme_minimal() +
    labs(title = paste("Correlation Heatmap for", condition_name), fill = "Correlation") +
...
p_correlation_heatmap <- ggplot(to_keep, aes(x = Variable1, y = Variable2, fill = Correlation)) +
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  theme_minimal() +
  labs(title = "Lower Triangle Correlation Matrix Heatmap", fill = "Correlation") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r possum-correlation-grouped1, eval=TRUE}
#
# THIS CHUNK WAS EXPLORATORY AND IS NOT USED
#
# Define categorical variables (one-hot site variables and sex)
#
categorical_vars <- c(one_hot_columns, "sex")
...
```{r possum-correlation-grouped2, eval=TRUE}
#
# THIS CHUNK WAS EXPLORATORY AND IS NOT USED
#
# Define categorical variables
site_columns <- one_hot_columns  # One-hot site columns
sex_values <- c(0, 1)            # Values for sex

# Function to generate correlation matrix and heatmap for a subset
generate_correlation_heatmap <- function(subset_name, subset_data) {
  # Select numeric features excluding categorical variables
  pnumerics <- subset_data |>
    select(where(is.numeric), -all_of(site_columns), -sex)
  ...
  p_correlation_heatmap <- ggplot(to_keep, aes(x = Variable1, y = Variable2, fill = Correlation)) +
    ...
    scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
    theme_minimal() +
    labs(title = paste("Correlation Heatmap for", subset_name), fill = "Correlation") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Loop through sex values (0 = Male, 1 = Female)
for (sex_val in sex_values) {
  
  # Loop through site variables, creating one plot per site
  for (site_var in site_columns) {
    subset_name <- paste("Site:", site_var, "= 1 | Sex =", sex_val)
    subset_data <- possums_1 |> filter(!!sym(site_var) == 1, sex == sex_val)
  ...
  # Create an additional graph for "No Site" (all sites = 0)
  subset_name <- paste("Site: None (All 0s) | Sex =", sex_val)
  subset_data <- possums_1 %>% filter(rowSums(select(., all_of(site_columns))) == 0, sex == sex_val)
...
# Select the top predictors (change number as needed, e.g., top 10)
num_features <- 10  # Adjust as necessary
