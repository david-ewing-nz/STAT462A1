...
```{r init-possums-analysis, include=T , eval=T}
...
possums_tmp  <- possums_1 |> relocate(age, .after = last_col()) # age is last
pnumerics    <- possums_tmp |> select(where(is.numeric))        # only use numeric values
...
if (length(ppnames) == 0) {
  pformula   <- as.formula("age ~ 1")                           # intercept-only 
} else {
  pformula   <- as.formula(paste("age ~", paste(ppnames, collapse=" + ")))  # now we know the names of the columns to use. 
}
