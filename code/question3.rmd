...
Having preprocessed the dataset by handling missing values, encoding categorical variables, and standardising features, we now explore the numerical distributions. The goal is to identify potential transformations required to normalise skewed distributions and detect multicollinearity among predictors.
...
Before training the model, we identify the most important predictors using Ridge Regression. This method penalises large coefficients, reducing multicollinearity and improving generalisability. The top features selected will form the basis for our final predictive model.
...
We stratified by sex, however, the correlation analysis itself does not incorporate sex as a predictor, ensuring that only numerical variables are examined for inter-dependencies.
